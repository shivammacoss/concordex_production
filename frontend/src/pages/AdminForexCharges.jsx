...
const AdminForexCharges = () => {
  ...
  const fetchAccountTypes = async () => {
    try {
      const res = await adminFetch('/account-types/all')
  ...
  const fetchCharges = async () => {
    setLoading(true)
    try {
      const res = await adminFetch('/charges?segment=Forex')
      const data = await res.json()
      if (data.success) {
  ...
  const handleSave = async () => {
    try {
      const endpoint = editingCharge 
        ? `/charges/${editingCharge._id}`
        : '/charges'
  ...
  const handleDelete = async (chargeId) => {
    ...
    try {
      const res = await adminFetch(`/charges/${chargeId}`, { method: 'DELETE' })
      const data = await res.json()
      if (data.success) {
