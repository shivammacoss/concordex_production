# Concorddex VPS Deployment Guide
...
PORT=5001
NODE_ENV=production
MONGODB_URI=mongodb://localhost:27017/Concorddex
...
# Start with PM2
pm2 start server.js --name Concorddex-backend
...
npm install -g serve
cd /home/user/nalmi/frontend
pm2 start "serve -s dist -l 5173" --name Concorddex-frontend
```
...
# Configure nginx
sudo nano /etc/nginx/sites-available/Concorddex
```
...
Enable the site:
```bash
sudo ln -s /etc/nginx/sites-available/Concorddex /etc/nginx/sites-enabled/
sudo nginx -t
...
pm2 stop Concorddex-backend   # Stop backend
pm2 delete Concorddex-backend # Remove from PM2
```
...
### View logs
```bash
pm2 logs Concorddex-backend --lines 100
```
