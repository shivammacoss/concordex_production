...
// GET /api/trading-accounts - Base route (returns empty or requires userId)
router.get('/', async (req, res) => {
  res.status(400).json({ success: false, message: 'User ID required. Use /api/trading-accounts/user/:userId' })
})
...
router.post('/', async (req, res) => {
  try {
    ...
    console.log('[Create Account] Request:', { userId, accountTypeId, pin: pin ? '****' : 'missing' })

    // Validate PIN (4 digits)
    if (!pin || pin.length !== 4 || !/^\d{4}$/.test(pin)) {
      console.log('[Create Account] PIN validation failed')
      return res.status(400).json({ message: 'PIN must be exactly 4 digits' })
    }

    // Get account type
    const accountType = await AccountType.findById(accountTypeId)
    console.log('[Create Account] AccountType found:', accountType ? accountType.name : 'NOT FOUND')
    ...
    // Handle different leverage formats (string "1:100" vs number 100)
    let leverageStr = accountType.leverage
    if (!leverageStr && accountType.maxLeverage) {
      leverageStr = `1:${accountType.maxLeverage}`
    }
    if (!leverageStr) {
      leverageStr = '1:100' // Default fallback
    }
    ...
    const tradingAccount = new TradingAccount({
      ...
      exposureLimit: accountType.exposureLimit || 0,
