...
// GET /api/admin/trade/all - Get all B-Book trades with pagination (for admin dashboard)
// A-Book trades are excluded - they should only be viewed in the A-Book section
router.get('/all', async (req, res) => {
  try {
    const { status, limit = 20, offset = 0 } = req.query

    // Get B-Book users (bookType 'B' or null/undefined - default is B-Book)
    const bBookUsers = await User.find({ 
      $or: [{ bookType: 'B' }, { bookType: { $exists: false } }, { bookType: null }] 
    }).select('_id')
    const bBookUserIds = bBookUsers.map(u => u._id)

    let query = { userId: { $in: bBookUserIds } }
...
router.put('/modify/:tradeId', async (req, res) => {
  try {
    ...
    // Check if user is in A Book - cannot modify A Book trades
    const user = await User.findById(trade.userId)
    if (user && user.bookType === 'A') {
      return res.status(403).json({ 
        success: false, 
        message: 'Cannot modify trades for A Book users. A Book trades are processed through liquidity provider.' 
      })
    }
...
router.put('/edit/:tradeId', async (req, res) => {
  try {
    ...
    // Check if user is in A Book - cannot edit A Book trades
    const user = await User.findById(trade.userId)
    if (user && user.bookType === 'A') {
      return res.status(403).json({ 
        success: false, 
        message: 'Cannot edit trades for A Book users. A Book trades are processed through liquidity provider.' 
      })
    }
...
router.post('/close/:tradeId', async (req, res) => {
  try {
    ...
    // Check if user is in A Book - cannot close A Book trades manually
    const user = await User.findById(trade.userId)
    if (user && user.bookType === 'A') {
      return res.status(403).json({ 
        success: false, 
        message: 'Cannot close trades for A Book users. A Book trades are processed through liquidity provider.' 
      })
    }
...
// GET /api/admin/trades - Get B-Book trades with filters
// A-Book trades are excluded - they should only be viewed in the A-Book section
...
// GET /api/admin/trades/open - Get all open B-Book trades
// A-Book trades are excluded - they should only be viewed in the A-Book section
router.get('/trades/open', async (req, res) => {
  try {
    ...
    const trades = await Trade.find({ status: 'OPEN', userId: { $in: bBookUserIds } })
      .populate('userId', 'firstName email bookType')
