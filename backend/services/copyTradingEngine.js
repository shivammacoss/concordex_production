...
class CopyTradingEngine {
  ...
  async closeFollowerTrades(masterTradeId, masterClosePrice) {
    ...
    // Debug: Check all CopyTrade records for this masterTradeId
    const allCopyTrades = await CopyTrade.find({ masterTradeId })
    console.log(`[CopyTrade] Total CopyTrade records for master ${masterTradeId}: ${allCopyTrades.length}`)
    allCopyTrades.forEach(ct => {
      console.log(`[CopyTrade]   - CopyTrade ${ct._id}: status=${ct.status}, followerTradeId=${ct.followerTradeId}, copyMode=${ct.copyMode}`)
    })
    ...
    const results = await Promise.all(copyTrades.map(async (copyTrade) => {
      try {
        // Skip if no follower trade ID (failed copy)
        if (!copyTrade.followerTradeId) {
          console.log(`[CopyTrade] Skipping copy trade ${copyTrade._id} - no followerTradeId (failed copy)`)
          ...
          return {
            copyTradeId: copyTrade._id,
            status: 'SKIPPED',
            reason: 'No follower trade (failed copy)'
          }
        }
        
        console.log(`[CopyTrade] Closing follower trade ${copyTrade.followerTradeId} for copy trade ${copyTrade._id}`)
          ...
          try {
            ...
            if (master && master.approvedCommissionPercentage) {
              ...
              if (followerAccount && followerAccount.balance >= totalCommission) {
                ...
                // Create or update commission record (accumulate for same day)
    ...
    for (const key in groupedTrades) {
      ...
      try {
        ...
        if (followerAccount && followerAccount.balance >= totalCommission) {
          ...
          // Insufficient balance for commission - update or create failed record
          await CopyCommission.findOneAndUpdate(
            {
              masterId: group.masterId,
              followerId: group.followerId,
