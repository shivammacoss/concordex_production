...
class TradeEngine {
  ...
  async closeTrade(tradeId, currentBid, currentAsk, closedBy = 'USER', adminId = null) {
    ...
    // So realizedPnl should NOT include open commission (to avoid double-counting)
    // realizedPnl = rawPnl - swap - closeCommission (for balance update)
    ...
    try {
      await tradeRouter.routeTradeUpdate(trade, 'close')
    } catch (routeError) {
      console.error('[TradeEngine] Error routing trade close:', routeError)
    }
  ...
  async modifyTrade(tradeId, sl = null, tp = null, adminId = null) {
    ...
    // Update stopLoss/takeProfit fields
